...
export interface PathCardData {
  ...
  level: string;
  /** optional category/domain — shown in meta row when present (mirrors mock-interview card) */
  category?: string | null;
  /** number of courses across the path's milestones */
  courses: number;
  /** number of milestones (topics) */
  milestones: number;
  estimatedWeeks?: number;
  hoursPerWeek?: number;
  /** fallback duration label when weeks aren't set, e.g. "Self-paced" */
...
export function PathCard({
  ...
  const duration =
    path.estimatedWeeks && path.estimatedWeeks > 0
      ? `~${path.estimatedWeeks}w · ${path.hoursPerWeek ?? 0}h/wk`
      : path.estimatedTime || "Self-paced";

  const action = isCompleted
    ? { label: "Certificate", Icon: Award, outline: true }
    : path.enrolled
  ...
  return (
    ...
    >
      {/* Top row: (category ·) courses · milestones + bookmark */}
      <div className="flex items-start justify-between gap-1">
        <div className="flex items-center gap-1.5 min-w-0 text-[11px]">
          ...
          <span className="text-muted-foreground font-medium truncate">
            {path.courses} {path.courses === 1 ? "course" : "courses"}
          </span>
          <span className="text-muted-foreground/40 shrink-0">·</span>
          <span className="text-muted-foreground font-medium truncate">
            {path.milestones} {path.milestones === 1 ? "milestone" : "milestones"}
          </span>
        </div>
        {onToggleSave && (
          <button
